...
int fantasma_alta(Fantasma array[], int size, int* contadorID)                          //cambiar fantasma
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            utn_getInt(&array[posicion].varInt,"\ngetInt: ","\nError",1,10,1);           //mensaje + cambiar campo varInt
            utn_getFloat(&array[posicion].varFloat,"\ngetFloat: ","\nError",0,1,1);             //mensaje + cambiar campo varFloat
...
int fantasma_ordenarPorDobleCriterio(Fantasma array[],int size)                              //cambiar fantasma
{
    ...
    if(array!=NULL && size>=0)
    {
        do
        {
            ...
            for (i = 1; i < size-1; i++)
            {
                if((strcmp(array[i].varString,array[i+1].varString) > 0)
                		&& array[i].isEmpty==0 && array[i+1].isEmpty==0)
                {
                    flagSwap=1;
                    buffer = array[i];
                    array[i] = array[i+1];
                ...
                {
                    if((array[i].varFloat > array[i+1].varFloat)
                    	&& array[i].isEmpty==0 && array[i+1].isEmpty==0)
